...
{
	// Not empty square, so there is an enemy on that square
	if (board[move.TargetSquare.x][move.TargetSquare.y] != nullptr)
	{
		// Capture move
		PiecePosition targetSquare = move.TargetSquare;
		// Stun enemy in the square
		board[targetSquare.y][targetSquare.x]->AddEffect(Effect::Stun);
		...
		if (Witch::m_UpgradeLevel == 3)
		{
			...
			{
				PiecePosition hexSquare = sf::Vector2i(targetSquare.y, dx);
				if (IsCellInBounds(hexSquare, m_Board->GetSize()))
				{
					const auto& targetPiece = board[hexSquare.y][hexSquare.x];
					if (targetPiece.get() != nullptr && targetPiece->GetPieceColor() != m_Color)
					{
						targetPiece->AddEffect(Effect::Hex);
					}
				}
			}
		}
